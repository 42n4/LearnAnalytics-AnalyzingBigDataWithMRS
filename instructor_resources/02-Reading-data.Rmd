...
- how `RevoScaleR` functions can work with data in memory (`data.frame`) and data on disk
- data on disk can consist of flat files (such as CSV files), MRS's native XDF format, and it can be stored locally or in a distributed file system such as HDFS
...
Each raw CSV file is slightly less than 2 Gbs in size, so 6 months worth of it amounts to 12 Gbs. That's usually more than available memory on a single personal computer. A server can have much larger memory capacity, but if a server is used by many users at once, R can very quickly run out of memory.
